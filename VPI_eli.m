...
%%%% Iteration
dis = 1; tol =1e-05; % tolerance for stopping 
v_guess = zeros(2,num_k);
 its = 0;
tic
while dis > tol  %& its < maxits
    % compute the utility value for all possible combinations of k and k':
     [vfnH,pol_indxH]=max(ret_h + beta.*repmat(mtimes(A(1,:),v_guess),[num_k 1]),[],2);
    [vfnL,pol_indxL]=max(ret_l + beta.*repmat(mtimes(A(2,:),v_guess),[num_k 1]),[],2);
   vfn=[vfnH'; vfnL'];
